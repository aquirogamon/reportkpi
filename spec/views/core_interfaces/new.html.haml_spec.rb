...
RSpec.describe "core_interfaces/new", type: :view do
  before(:each) do
    assign(:core_interface, CoreInterface.new(
      :device => "MyString",
      :port => "MyString",
      :servicio => "MyString",
      :gbpsrx => 1.5,
  ...
  it "renders new core_interface form" do
    render

    assert_select "form[action=?][method=?]", core_interfaces_path, "post" do
