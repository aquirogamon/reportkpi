...
RSpec.describe "preagg_interfaces/new", type: :view do
  before(:each) do
    assign(:preagg_interface, PreaggInterface.new(
      :device => "MyString",
      :port => "MyString",
      :servicio => "MyString",
      :gbpsrx => 1.5,
  ...
  it "renders new preagg_interface form" do
    render

    assert_select "form[action=?][method=?]", preagg_interfaces_path, "post" do
